...
// ToDo: remove this comment for alex_nodes (for P also)
    // const T& key() const { return cur_leaf_->get_key(cur_idx_); }
    const T key() const { return cur_leaf_->get_key(cur_idx_); }

    P& payload() { return cur_leaf_->get_payload(cur_idx_); }
  ...
  class ConstIterator {
   public:
    ...
    const P payload()  { return cur_leaf_->get_payload(cur_idx_); }
