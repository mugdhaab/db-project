...
class AlexDataNode : public AlexNode<T, P> {
 public:
...
#if ALEX_DATA_NODE_SEP_ARRAYS
    vector<T> key_slots_(0, data_capacity_);
        // new (key_allocator().allocate(data_capacity_)) T[data_capacity_];
    vector<T> payload_slots_(0, data_capacity_);
        // new (payload_allocator().allocate(data_capacity_)) P[data_capacity_];
#else
  vector<T> data_slots_(0, data_capacity_);
    // data_slots_ =
    //     new (value_allocator().allocate(data_capacity_)) V[data_capacity_];
#endif
  }
  ...
  void resize(double target_density, bool force_retrain = false,
...
#if ALEX_DATA_NODE_SEP_ARRAYS
    vector<T> new_key_slots(0, new_data_capacity);
        // new (key_allocator().allocate(new_data_capacity)) T[new_data_capacity];
    vector<P> new_payload_slots(0, new_data_capacity);
    // new (payload_allocator().allocate(new_data_capacity))
        // P[new_data_capacity];
#else
    vector<V> new_data_slots(0, new_data_capacity);
    // = new (value_allocator().allocate(new_data_capacity))
        // V[new_data_capacity];
#endif
    ...
    // payload_allocator().deallocate(payload_slots_, data_capacity_);
    key_slots_.clear();
    key_slots_.shrink_to_fit();
    payload_slots_.clear();
    payload_slots_.shrink_to_fit();
#else
    value_allocator().deallocate(data_slots_, data_capacity_);
#endif
    bitmap_allocator().deallocate(bitmap_, bitmap_size_);
